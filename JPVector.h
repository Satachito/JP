...
namespace JP {
	...
	namespace Accelerate {
		...
		Vector 	: vVector< F > {
			...
			:	vVector< F >( new F[ n * s ](), n, s ) {
			}
			Vector( F* p, size_t n, size_t s = 1 )
			:	vVector< F >( new F[ n * s ], n, s ) {
				for ( auto i = 0; i < n * s; i += s ) vVector< F >::m[ i ] = p[ i ];
			}
			Vector( F p, size_t n, size_t s = 1 )
			:	vVector< F >( new F[ n * s ], n, s ) {
				for ( auto i = 0; i < n * s; i += s ) vVector< F >::m[ i ] = p;
			}
			Vector( F( *p )(), size_t n, size_t s = 1 )
			:	vVector< F >( new F[ n * s ], n, s ) {
				for ( auto i = 0; i < n * s; i += s ) vVector< F >::m[ i ] = p();
		...
		template	< typename F >	Vector< F >			operator -( const vVector< F >& p ) {
			Vector< F > v( p );
		...
		template	< typename F >	Vector< F >			Abs( const vVector< F >& p ) {
			Vector< F > v( p );
		...
		template	< typename F >	Vector< F >			Rec( const vVector< F >& p ) {
		...
		template	< typename F >	Vector< F >			Exp( const vVector< F >& p ) {
			assert( p.s == 1 );	//	Don't support strided
			Vector< F > v( p );
			Exp( p.m, v.m, (int)v.n );
			return v;
		...
		template	< typename F >	F					Dot( const vVector< F >& l, const vVector< F >& r ) {
			assert( l.n == r.n );
			F	v = 0;
