...
namespace JP {
	...
	template	< typename F >	Vector< F >			Rec				( const vVector< F >& p									) {	assert( p.s == 1 );	Vector< F > v( p );		_Rec		( p.m, v.m, (int)v.n );								return v; }
	template	< typename F >	Vector< F >			Exp				( const vVector< F >& p									) {	assert( p.s == 1 );	Vector< F > v( p );		_Exp		( p.m, v.m, (int)v.n );								return v; }
