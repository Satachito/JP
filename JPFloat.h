...
namespace JP {
	...
	namespace CPU {
		template < typename F >	void	_Clr		( F* p, vDSP_Stride sP, vDSP_Length pLength )															{ for ( auto i = 0; i < pLength; i++ ) p[ i * sP ] = 0; }
		template < typename F >	void	_Ramp		( F pInit, F pStep, F* p, vDSP_Stride sP, vDSP_Length pLength )											{ for ( auto i = 0; i < pLength; i++ ) p[ i * sP ] = pInit + pStep * i; }
		template < typename F >	void	_Sve		( const F* p, vDSP_Stride sP, F& v, vDSP_Length pLength )												{ v = 0; for ( auto i = 0; i < pLength; i++ ) v += p[ i * sP ]; }
		template < typename F >	void	_Mean		( const F* p, vDSP_Stride sP, F& v, vDSP_Length pLength )												{ v = 0; for ( auto i = 0; i < pLength; i++ ) v += p[ i * sP ]; v /= pLength; }
		template < typename F >	void	_Max		( const F* p, vDSP_Stride sP, F& v, vDSP_Length pLength )												{ v = *p; for ( auto i = 1; i < pLength; i++ ) if ( p[ i * sP ] > v ) v = p[ i * sP ]; }
		template < typename F >	void	_Min		( const F* p, vDSP_Stride sP, F& v, vDSP_Length pLength )												{ v = *p; for ( auto i = 1; i < pLength; i++ ) if ( p[ i * sP ] < v ) v = p[ i * sP ]; }
		template < typename F >	void	_Svemg		( const F* p, vDSP_Stride sP, F& v, vDSP_Length pLength )												{ v = 0; for ( auto i = 0; i < pLength; i++ ) v += p[ i * sP ] < 0 ? -p[ i * sP ] : p[ i * sP ] ; }
		template < typename F >	void	_Svesq		( const F* p, vDSP_Stride sP, F& v, vDSP_Length pLength )												{ v = 0; for ( auto i = 0; i < pLength; i++ ) v += p[ i * sP ] * p[ i * sP ]; }
		template < typename F >	void	_Distancesq	( const F* l, vDSP_Stride sL, const F* r, vDSP_Stride sR, F& v, vDSP_Length pLength )					{ v = 0; for ( auto i = 0; i < pLength; i++ ) { auto w = l[ i * sL ] - r[ i * sR ]; v += w * w; } }
		template < typename F >	void	_Neg		( const F* p, vDSP_Stride sP, F* v, vDSP_Stride sV, vDSP_Length pLength )								{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = -p[ i * sP ]; }
		template < typename F >	void	_Abs		( const F* p, vDSP_Stride sP, F* v, vDSP_Stride sV, vDSP_Length pLength )								{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = p[ i * sP ] < 0 ? -p[ i * sP ] : p[ i * sP ]; }
		template < typename F >	void	_Rec		( const F* p, F* v, int pLength )																		{ vvrec				( v, p, &pLength ); }
		template < typename F >	void	_Exp		( const F* p, F* v, int pLength )																		{ vvexp				( v, p, &pLength ); }
		template < typename F >	void	_Add		( const F* l, vDSP_Stride sL, const F* r, vDSP_Stride sR, F* v, vDSP_Stride sV, vDSP_Length pLength )	{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = l[ i * sL ] + r[ i * sR ]; }
		template < typename F >	void	_Add		( const F* l, vDSP_Stride sL, const F& r, F* v, vDSP_Stride sV, vDSP_Length pLength )					{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = l[ i * sL ] + r; }
		template < typename F >	void	_Sub		( const F* l, vDSP_Stride sL, const F* r, vDSP_Stride sR, F* v, vDSP_Stride sV, vDSP_Length pLength )	{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = l[ i * sL ] - r[ i * sR ]; }
		template < typename F >	void	_Mul		( const F* l, vDSP_Stride sL, const F* r, vDSP_Stride sR, F* v, vDSP_Stride sV, vDSP_Length pLength )	{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = l[ i * sL ] * r[ i * sR ]; }
		template < typename F >	void	_Mul		( const F* l, vDSP_Stride sL, const F& r, F* v, vDSP_Stride sV, vDSP_Length pLength )					{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = l[ i * sL ] * r; }
		template < typename F >	void	_Div		( const F* l, vDSP_Stride sL, const F* r, vDSP_Stride sR, F* v, vDSP_Stride sV, vDSP_Length pLength )	{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = l[ i * sL ] / r[ i * sR ]; }
		template < typename F >	void	_Div		( const F* l, vDSP_Stride sL, const F& r, F* v, vDSP_Stride sV, vDSP_Length pLength )					{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = l[ i * sL ] / r; }
		template < typename F >	void	_Div		( const F& l, const F* r, vDSP_Stride sR, F* v, vDSP_Stride sV, vDSP_Length pLength )					{ for ( auto i = 0; i < pLength; i++ ) v[ i * sV ] = l / r[ i * sR ]; }
		template < typename F >	void	_Dot		( const F* l, vDSP_Stride sL, const F* r, vDSP_Stride sR, F& v, vDSP_Length pLength )					{ v = 0; for ( auto i = 0; i < pLength; i++ ) v += l[ i * sL ] * r[ i * sR ]; }
		template < typename F >	void	_Trans		( const F* p, vDSP_Stride sP, F* v, vDSP_Stride sV, vDSP_Length nR, vDSP_Length nC )					{ vDSP_mtransD		( p, sP, v, sV, nR, nC ); }
	}
}
