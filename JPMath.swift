...
Matrix: NSObject {
	...
	func
	Rows( _ p: CountableRange<Int> ) -> Matrix {
		let w = Array( u[ p.lowerBound * numC ..< p.upperBound * numC ] )
		return Matrix( p.count, numC, w )
	}
	func
	Rows( _ p: CountableClosedRange<Int> ) -> Matrix {
		let w = Array( u[ p.lowerBound * numC ... p.upperBound * numC ] )
		return Matrix( p.count, numC, w )
	}
	func
	Cols( _ p: CountableRange<Int> ) -> Matrix {
	...
	func
	Cols( _ p: CountableClosedRange<Int> ) -> Matrix {
		let	v = Matrix( numR, p.count )
		for i in 0 ..< numR {
			let	wVS = i * p.count
			let	wSS = i * numC
...
func
RandomMatrix( _ numR: Int, _ numC: Int ) -> Matrix {
	var	v = [ Float ]( repeating: 0, count: numR * numC )
	for i in 0 ..< v.count { v[ i ] = Float.random( in: 0 ..< 1 ) }
